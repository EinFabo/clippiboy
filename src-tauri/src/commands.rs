...
pub fn set_config(
    ...
    // Size or place of the console changed in the settings while it happens to
    // be open: it should not wait for the next opening.
    if previous.console_scale != next.console_scale {
        crate::console::rescale(&app);
    }
...
/// that clip. The console closes behind it — the question was "let me see this
/// properly", and that happens in the window.
#[tauri::command]
pub fn show_clip_in_app(app: tauri::AppHandle, id: String) -> Result<()> {
    use tauri::Emitter;

    crate::console::close(&app);
    crate::tray::show_main_window(&app);
    app.emit("focus-clip", id).map_err(|e| e.to_string())
}
